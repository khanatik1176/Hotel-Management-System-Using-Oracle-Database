...
namespace HotelManagementSystem
{
    public partial class HelplessPeople : Form
    {
        ...
        private void PopulateGridView(string sql = "select * from HelplessPeopleLists order by helplesspeoplelistid")
        {
        ...
        private void btnHelplesspeopleAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    var sql = @"insert into HelplessPeopleLists values('" + this.newID + "', '" + this.txtHelplesspeoplename.Text + "')";
        ...
        private void btnHelplessPeopleDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var sql = "delete from HelplessPeopleLists where HelplessPeopleName = '" + name + "'";
        ...
        private void btnHelplessPeopleUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    var query = "select * from HelplessPeoplelists where HelplessPeopleName = '" + name + "'";
                    ...
                    if (dst.Tables[0].Rows.Count == 1)
                    {
                        ...
                        var kt = @"update HelplessPeoplelists
                               
                                
                                 set HelplessPeopleName = '" + this.txtHelplesspeoplename.Text + @"',
                                 HelplessPeoplelistID = '" + this.txtHelplessPeopleID.Text + @"'
                                 where HelplessPeoplelistID = '" + id + "'";
        ...
        private void txtHelplessPeopleSearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                string search = "select * from HelplessPeopleLists where HelplesspeopleName = '" + this.txtHelplessPeopleSearch.Text + "'";
        ...
        private void GenerateID()
        {
            try
            {
                var sql = @"select HelplessPeoplelistID from HelplessPeopleLists Order by HelplessPeoplelistID DESC";
                var ds = this.Da.ExecuteQuery(sql);
                string oldID = ds.Tables[0].Rows[0]["HelplessPeoplelistID"].ToString();
